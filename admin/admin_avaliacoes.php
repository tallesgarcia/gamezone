...
// Filtro de nota
$filtroNota = $_GET['nota'] ?? '';
$condicao = '';
$param = [];

if ($filtroNota !== '' && is_numeric($filtroNota)) {
    $condicao = 'WHERE a.nota = ?';
    $param[] = $filtroNota;
}
$query = "SELECT a.id, a.nota, a.comentario, a.criado_em,
...
<head>
  <meta charset="UTF-8">
  <title>Admin | Avaliações | GameZone</title>
...
<div class="fixed top-0 left-0 h-full w-64 bg-gray-800 text-white shadow-lg">
  ...
  <nav class="flex flex-col gap-2 mt-4 px-4">
    ...
    <a href="admin_avaliacoes.php" class="text-indigo-400 font-semibold">⭐ Avaliações</a>
...
<div class="ml-64 pt-20 p-6 max-w-7xl mx-auto">
  <h2 class="text-2xl font-bold mb-4">Avaliações dos Usuários</h2>

  <form method="GET" class="mb-4 flex gap-4 items-center">
    <label for="nota" class="text-sm font-semibold">Filtrar por nota:</label>
    <select name="nota" id="nota" class="text-sm p-1 rounded border border-gray-300">
      <option value="">Todas</option>
      <?php for ($i = 1; $i <= 5; $i++): ?>
        <option value="<?= $i ?>" <?= $filtroNota == $i ? 'selected' : '' ?>><?= $i ?> estrela<?= $i > 1 ? 's' : '' ?></option>
      <?php endfor; ?>
    </select>
    <button type="submit" class="bg-indigo-600 text-white px-3 py-1 rounded hover:bg-indigo-700">Filtrar</button>
  </form>

  <?php if (!empty($avaliacoes)): ?>
<table class="w-full border text-sm bg-white shadow rounded table-fixed">
  <thead class="bg-gray-200">
    <tr>
      <th class="p-2 border w-12 text-left">ID</th>
      <th class="p-2 border w-32 text-left">Usuário</th>
      <th class="p-2 border w-20 text-center">Nota</th>
      <th class="p-2 border w-96 text-left">Comentário</th>
      <th class="p-2 border w-40 text-left">Data</th>
      <th class="p-2 border w-32 text-center">Ações</th>
    </tr>
  </thead>
  <tbody>
    <?php foreach ($avaliacoes as $av): ?>
      <tr class="hover:bg-gray-50 align-top">
        <td class="border p-2 align-top"><?= $av['id'] ?></td>
        <td class="border p-2 align-top"><?= htmlspecialchars($av['usuario_nome']) ?></td>
        <td class="border p-2 align-top text-center"><?= $av['nota'] ?> ★</td>
        <td class="border p-2 align-top break-words"><?= nl2br(htmlspecialchars($av['comentario'])) ?></td>
        <td class="border p-2 align-top"><?= date("d/m/Y H:i", strtotime($av['criado_em'])) ?></td>
        <td class="border p-2 text-center align-top">
          <form action="acoes/excluir_avaliacao.php" method="POST" onsubmit="return confirm('Tem certeza que deseja excluir esta avaliação?')">
            <input type="hidden" name="id" value="<?= $av['id'] ?>">
            <button class="bg-red-600 text-white px-2 py-1 text-xs rounded hover:bg-red-700">Excluir</button>
          </form>
        </td>
      </tr>
    <?php endforeach; ?>
