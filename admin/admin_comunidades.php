...
$stmt = $conn->prepare("SELECT * FROM comunidades ORDER BY membros DESC, nome ASC");
$stmt->execute();
$comunidades = $stmt->get_result();
...
<head>
  <meta charset="UTF-8">
  <title>Gerenciar Comunidades | Admin | GameZone</title>
...
<body class="bg-gray-100 flex">
  ...
  <nav class="flex flex-col gap-2 mt-4 px-4">
    ...
    <a href="admin_comunidades.php" class="text-indigo-400 font-semibold">🌐 Comunidades</a>
...
<nav class="fixed top-0 left-0 right-0 z-30 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 h-16 flex items-center justify-between px-6 shadow-sm">
  ...
  <main class="ml-64 pt-20 p-6 w-full">
    <h1 class="text-2xl font-bold text-gray-800 mb-6">🌐 Comunidades</h1>

    <div class="overflow-x-auto">
      <table class="min-w-full bg-white shadow rounded">
        <thead class="bg-gray-200 text-gray-700 text-sm text-left">
          <tr>
            <th class="py-3 px-4">#</th>
            <th class="py-3 px-4">Nome</th>
            <th class="py-3 px-4">Membros</th>
            <th class="py-3 px-4">Popular</th>
            <th class="py-3 px-4">Ações</th>
          </tr>
        </thead>
        <tbody class="text-sm text-gray-800">
          <?php while ($comunidade = $comunidades->fetch_assoc()): ?>
            <tr class="border-t hover:bg-gray-50">
              <td class="py-2 px-4"><?= $comunidade['id'] ?></td>
              <td class="py-2 px-4"><?= htmlspecialchars($comunidade['nome']) ?></td>
              <td class="py-2 px-4"><?= $comunidade['membros'] ?></td>
              <td class="py-2 px-4">
                <?= $comunidade['popular'] ? '<span class="text-green-600 font-medium">Sim</span>' : '<span class="text-gray-500">Não</span>' ?>
              </td>
              <td class="py-2 px-4">
                <div class="flex flex-row flex-wrap gap-2 items-center">
                  <a href="../pages/comunidade/ver_comunidade.php?id=<?= $comunidade['id'] ?>"
                    target="_blank"
                    class="bg-indigo-600 hover:bg-indigo-500 text-white px-3 py-1 rounded text-xs">Ver</a>


                  <?php if ($comunidade['popular']): ?>
                    <a href="acoes/desmarcar_popular_comunidade.php?id=<?= $comunidade['id'] ?>"
                      class="bg-yellow-500 hover:bg-yellow-400 text-white px-3 py-1 rounded text-xs">Remover Popular</a>
                  <?php else: ?>
                    <a href="acoes/marcar_popular_comunidade.php?id=<?= $comunidade['id'] ?>"
                      class="bg-green-600 hover:bg-green-500 text-white px-3 py-1 rounded text-xs">Tornar Popular</a>
                  <?php endif; ?>

                  <a href="acoes/excluir_comunidade.php?id=<?= $comunidade['id'] ?>"
                    onclick="return confirm('Deseja excluir esta comunidade?')"
                    class="bg-red-600 hover:bg-red-500 text-white px-3 py-1 rounded text-xs">Excluir</a>
                </div>
              </td>
            </tr>
          <?php endwhile; ?>
