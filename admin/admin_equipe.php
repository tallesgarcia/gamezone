...
$stmt = $conn->prepare("SELECT * FROM equipe ORDER BY ordem ASC");
$stmt->execute();
$equipe = $stmt->get_result();
...
<head>
  <meta charset="UTF-8">
  <title>Gerenciar Equipe | Admin | GameZone</title>
...
<div class="fixed top-0 left-0 h-full w-64 bg-gray-800 text-white shadow-lg">
  ...
  <nav class="flex flex-col gap-2 mt-4 px-4">
    ...
    <a href="admin_equipe.php" class="text-indigo-400 font-semibold">🧑‍💼 Equipe</a>
...
<main class="ml-64 pt-20 p-6 w-full">
  <div class="flex justify-between items-center mb-6">
    <h1 class="text-2xl font-bold text-gray-800">🧑‍💼 Membros da Equipe</h1>
    <a href="equipe_adicionar.php" class="bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2 rounded text-sm">+ Novo Membro</a>
  </div>
  ...
  <div class="overflow-x-auto">
    <table class="min-w-full bg-white shadow rounded">
      ...
      <tbody class="text-sm text-gray-800">
        <?php if ($equipe->num_rows === 0): ?>
          <tr><td colspan="7" class="py-4 px-4 text-center text-gray-500">Nenhum membro cadastrado.</td></tr>
        <?php else: ?>
          <?php while ($membro = $equipe->fetch_assoc()): ?>
            <tr class="border-t hover:bg-gray-50">
              <td class="py-2 px-4"><?= $membro['id'] ?></td>
              <td class="py-2 px-4 font-semibold"><?= htmlspecialchars($membro['nome']) ?></td>
              <td class="py-2 px-4"><?= htmlspecialchars($membro['cargo']) ?></td>
              <td class="py-2 px-4"><?= htmlspecialchars($membro['descricao']) ?></td>
              <td class="py-2 px-4">
                <?php if ($membro['foto_url']): ?>
                  <img src="<?= htmlspecialchars($membro['foto_url']) ?>" alt="Foto" class="w-10 h-10 rounded-full">
                <?php else: ?>
                  <span class="text-gray-400 italic">Sem foto</span>
                <?php endif; ?>
              </td>
              <td class="py-2 px-4 space-y-1">
                <?php if ($membro['github_url']): ?>
                  <a href="<?= $membro['github_url'] ?>" target="_blank" class="block text-blue-600 hover:underline text-xs">GitHub</a>
                <?php endif; ?>
                <?php if ($membro['linkedin_url']): ?>
                  <a href="<?= $membro['linkedin_url'] ?>" target="_blank" class="block text-blue-600 hover:underline text-xs">LinkedIn</a>
                <?php endif; ?>
              </td>
              <td class="py-2 px-4 flex gap-2">
                <a href="equipe_editar.php?id=<?= $membro['id'] ?>" class="bg-blue-600 hover:bg-blue-500 text-white px-3 py-1 rounded text-xs">Editar</a>
                <button 
...
<div id="modal-excluir" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center">
  <div class="bg-white p-6 rounded shadow-lg w-96">
    <h2 class="text-lg font-bold text-gray-800 mb-4">Tem certeza que deseja excluir?</h2>
    ...
    <div class="flex justify-end gap-2">
      ...
      <a id="linkExcluir" href="#" class="bg-red-600 text-white px-4 py-2 rounded">Excluir</a>
    </div>
  </div>
</div>
...
function confirmarExclusao(id) {
  document.getElementById('linkExcluir').href = 'acoes/equipe_excluir.php?id=' + id;
