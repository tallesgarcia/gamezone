...
<head>
  <meta charset="UTF-8">
  <title>Painel Administrativo | GameZone</title>
...
<body class="bg-gray-100 flex">
  ...
  <nav class="flex flex-col gap-2 mt-4 px-4">
    <a href="admin_painel.php" class="text-indigo-400 font-semibold">📊 Painel</a>
...
$totalComunidades = 0;
$result = $conn->query("SELECT COUNT(*) AS total FROM comunidades");
if ($result && $row = $result->fetch_assoc()) {
  $totalComunidades = $row['total'];
}

// Total de vendas (simulado)
$totalVendas = 0.00;
$result = $conn->query("SELECT SUM(valor_total) AS total FROM vendas");
if ($result && $row = $result->fetch_assoc()) {
  $totalVendas = $row['total'] ?? 0.00;
}
?>

<main class="ml-64 pt-20 p-6 w-full">
  <h1 class="text-2xl font-bold text-gray-800 mb-4">📊 Visão Geral</h1>

  <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
    <div class="bg-white p-4 shadow rounded">
      <p class="text-gray-500">Usuários Cadastrados</p>
      <h2 class="text-xl font-bold"><?= $totalUsuarios ?></h2>
    </div>

    <div class="bg-white p-4 shadow rounded">
      <p class="text-gray-500">Comunidades Criadas</p>
      <h2 class="text-xl font-bold"><?= $totalComunidades ?></h2>
    </div>

    <div class="bg-white p-4 shadow rounded">
      <p class="text-gray-500">Total de Vendas</p>
      <h2 class="text-xl font-bold">R$ <?= number_format($totalVendas, 2, ',', '.') ?></h2>
    </div>
  </div>
</main>
