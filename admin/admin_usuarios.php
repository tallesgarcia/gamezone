...
$usuarios = [];
$stmt = $conn->prepare("SELECT id, nome, email, tipo_usuario, data_cadastro FROM usuarios ORDER BY data_cadastro DESC");
if ($stmt) {
  $stmt->execute();
...
<head>
  <meta charset="UTF-8">
  <title>Gerenciar Usuários | Admin | GameZone</title>
...
<div class="fixed top-0 left-0 h-full w-64 bg-gray-800 text-white shadow-lg">
  ...
  <nav class="flex flex-col gap-2 mt-4 px-4">
    ...
    <a href="admin_usuarios.php" class="text-indigo-400 font-semibold">👥 Usuários</a>
      ...
      <table class="min-w-full bg-white rounded shadow">
        <thead class="bg-gray-200 text-gray-700 text-sm">
          <tr>
            ...
            <th class="py-3 px-4 text-left">Cadastro</th>
        ...
        <tbody class="text-sm text-gray-800">
          <?php foreach ($usuarios as $user): ?>
            <tr class="border-t hover:bg-gray-50">
              ...
              <td class="py-2 px-4">
                <?= $user['tipo_usuario'] === 'admin' ? 'Administrador' : 'Comum' ?>
              </td>
              <td class="py-2 px-4">
                <?= date('d/m/Y', strtotime($user['data_cadastro'])) ?>
              </td>
              <td class="py-2 px-4 flex gap-2 flex-wrap">
              <?php if ($user['tipo_usuario'] === 'comum'): ?>
                <a href="acoes/promover_usuario.php?id=<?= $user['id'] ?>"
                  class="bg-indigo-600 hover:bg-indigo-500 text-white px-3 py-1 rounded text-xs font-medium transition">
                  Promover
                </a>
              <?php elseif ($user['id'] !== $_SESSION['user_id']): ?>
                <a href="acoes/rebaixar_usuario.php?id=<?= $user['id'] ?>"
                  class="bg-yellow-500 hover:bg-yellow-400 text-white px-3 py-1 rounded text-xs font-medium transition">
                  Rebaixar
                </a>
              <?php endif; ?>

              <?php if ($user['id'] !== $_SESSION['user_id']): ?>
                <a href="acoes/excluir_usuario.php?id=<?= $user['id'] ?>"
