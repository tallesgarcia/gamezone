...
// Pega produto atual
$stmt = $conn->prepare("SELECT id, nome, descricao, preco, tipo, imagem, ativo FROM produtos WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nome === '' || $preco < 0) {
        ...
        $novaImagem = $produto['imagem']; // mantém a atual por padrão
        ...
        if (!$mensagem) {
            $stmtUp = $conn->prepare("UPDATE produtos SET nome=?, descricao=?, preco=?, tipo=?, imagem=?, ativo=? WHERE id=?");
            if (!$stmtUp) {
                $mensagem = "Erro no banco: " . $conn->error;
            } else {
                // bind_param types: s s d s s i i => "ssdsiii" but imagem is string/blob; to match previous pattern:
                // We'll use "ssdssii" where the two 'i' are ativo and id. But careful with order.
                // Correct order: nome(s), descricao(s), preco(d), tipo(s), imagem(s), ativo(i), id(i)
                $stmtUp->bind_param("ssdssii", $nome, $descricao, $preco, $tipo, $novaImagem, $ativo, $id);

                if ($novaImagem !== $produto['imagem'] && $novaImagem !== null) {
                    // imagem at param index 4 (0-based)
                    $stmtUp->send_long_data(4, $novaImagem);
                }

                if ($stmtUp->execute()) {
                    $stmtUp->close();
                    header("Location: admin_produtos.php?edit=ok");
                    exit;
                } else {
                    $mensagem = "Erro ao atualizar: " . $stmtUp->error;
                    $stmtUp->close();
                }
            }
        }
...
<head>
<meta charset="utf-8">
<title>Editar Produto - Admin | GameZone</title>
...
<main class="ml-64 pt-20 p-6">
  ...
  <form method="POST" enctype="multipart/form-data" class="bg-gray-800 p-6 rounded w-full max-w-xl">
    ...
    <input name="nome" required value="<?= htmlspecialchars($produto['nome']) ?>" class="w-full p-2 rounded bg-gray-700 mb-3">

    <label class="block mb-2 font-semibold">Descrição</label>
    <textarea name="descricao" class="w-full p-2 rounded bg-gray-700 mb-3"><?= htmlspecialchars($produto['descricao']) ?></textarea>
    ...
    <select name="tipo" class="w-full p-2 rounded bg-gray-700 mb-3">
      ...
      <option value="assinatura" <?= $produto['tipo']=="assinatura"?"selected":"" ?>>Assinatura</option>
    </select>

    <label class="block mb-2 font-semibold">Preço (R$) *</label>
    <input name="preco" type="number" step="0.01" required value="<?= $produto['preco'] ?>" class="w-full p-2 rounded bg-gray-700 mb-3">

    <div class="mb-3">
      <label class="block mb-2 font-semibold">Imagem atual:</label>
      <?php if (!empty($produto['imagem'])):
        $finfo = new finfo(FILEINFO_MIME_TYPE);
        $mime = $finfo->buffer($produto['imagem']) ?: 'image/jpeg';
        $base64 = base64_encode($produto['imagem']);
      ?>
        <img src="data:<?= htmlspecialchars($mime) ?>;base64,<?= $base64 ?>" class="w-32 h-32 object-cover rounded mb-3">
      <?php else: ?>
        <div class="w-32 h-32 bg-gray-600 rounded mb-3 flex items-center justify-center text-gray-300">sem imagem</div>
      <?php endif; ?>
    </div>

    <label class="block mb-2 font-semibold">Nova imagem (opcional, max 2MB)</label>
    <input name="imagem" type="file" accept="image/*" class="mb-3">

    <div class="flex items-center gap-3 mb-3">
      <label class="inline-flex items-center">
        <input type="checkbox" name="ativo" <?= $produto['ativo'] ? 'checked' : '' ?> class="mr-2">
    ...
    <div class="flex justify-end">
      ...
      <button class="px-4 py-2 rounded bg-indigo-600 hover:bg-indigo-500">Salvar Alterações</button>
    </div>
  </form>
</main>
<script>
