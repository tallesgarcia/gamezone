...
if (isset($_GET['id'])) {
    ...
    $stmt = $conn->prepare("SELECT preco FROM produtos WHERE id = ? AND ativo = 1");
    ...
    if ($resultado->num_rows > 0) {
        ...
        $venda = $conn->prepare("INSERT INTO vendas (id, produto_id, valor_total) VALUES (?, ?, ?)");
        $venda->bind_param("iid", $id, $produto_id, $valor_total);
        $venda->execute();
...
header("Location: loja.php?erro=1");
exit;
?>