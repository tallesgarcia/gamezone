...
$stmt->execute();
$bloqueados = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
...
<head>
  <meta charset="UTF-8">
  <title>Bloqueados | GameZone</title>
...
<nav class="fixed top-0 left-0 right-0 z-30 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 h-16 flex items-center justify-between px-6 shadow-sm">
  ...
  <div class="flex h-screen">
    ...
    <main class="flex-1 p-6 overflow-y-auto">
      <h1 class="text-2xl font-semibold mb-4">Usuários Bloqueados</h1>
      <?php if (empty($bloqueados)): ?>
        <p class="text-gray-400">Você não bloqueou ninguém.</p>
      <?php else: ?>
        <ul class="space-y-4">
          <?php foreach ($bloqueados as $u): ?>
            <li class="bg-gray-800 p-4 rounded-lg flex items-center justify-between">
              <div class="flex items-center space-x-4">
                <img src="<?= htmlspecialchars($u['avatar'] ?? 'default_avatar.png') ?>" class="w-10 h-10 rounded-full object-cover">
                <div>
                  <p class="font-semibold"><?= htmlspecialchars($u['nome']) ?></p>
                  <p class="text-sm text-gray-400"><?= htmlspecialchars($u['email']) ?></p>
                </div>
              </div>
              <form action="../../desbloquear_usuario.php" method="POST">
                <input type="hidden" name="bloqueado_id" value="<?= $u['id'] ?>">
                <button class="bg-green-600 hover:bg-green-700 px-3 py-1 rounded">Desbloquear</button>
              </form>
            </li>
          <?php endforeach; ?>
        </ul>
