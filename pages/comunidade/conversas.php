...
$sql = "SELECT u.id, u.nome 
        FROM amizades a 
        JOIN usuarios u ON (u.id = IF(a.usuario_id = ?, a.amigo_id, a.usuario_id))
...
$amigo_selecionado = $_GET['amigo_id'] ?? null;
...
<head>
  <meta charset="UTF-8">
  <title>Conversas | GameZone</title>
...
<!-- Sidebar -->
<div class="pt-16 w-64 bg-gray-800 p-4 overflow-y-auto scrollbar">
  <h2 class="text-xl font-bold mb-4">💬 Conversas</h2>
  <ul class="space-y-2">
    <?php foreach ($amigos as $amigo): ?>
      <li>
        <a href="?amigo_id=<?= $amigo['id'] ?>" class="block px-3 py-2 rounded hover:bg-gray-700 <?= $amigo_selecionado == $amigo['id'] ? 'bg-gray-700' : '' ?>">
...
<div class="flex-1 flex flex-col h-full">
  <div class="p-4 border-b border-gray-700">
    <?php if ($amigo_selecionado): ?>
      <h1 class="text-lg font-semibold">Conversando com: 
        <?= htmlspecialchars($conn->query("SELECT nome FROM usuarios WHERE id = " . intval($amigo_selecionado))->fetch_assoc()['nome'] ?? 'Desconhecido') ?>
      </h1>
    <?php else: ?>
      <h1 class="text-lg font-semibold text-gray-400">Selecione um amigo para começar</h1>
    <?php endif; ?>
  </div>

  <!-- Mensagens -->
  <div id="mensagens" class="flex-1 p-4 overflow-y-auto scrollbar space-y-2 bg-gray-900"></div>

  <!-- Formulário -->
  <?php if ($amigo_selecionado): ?>
  <form id="formMensagem" class="p-4 border-t border-gray-700 flex gap-2">
  <input type="hidden" name="amigo_id" value="<?= $amigo_selecionado ?>">
  <input type="text" name="mensagem" placeholder="Digite sua mensagem..." class="flex-1 p-2 bg-gray-700 rounded text-white" required>
  <button type="submit" class="bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2 rounded">Enviar</button>
  </form>
  <?php endif; ?>
</div>
...
function carregarMensagens() {
  const idAmigo = <?= $amigo_selecionado ? intval($amigo_selecionado) : 'null' ?>;
  if (!idAmigo) return;

  $.get('get_mensagens.php', { amigo_id: idAmigo }, function(data) {
    $('#mensagens').html(data);
...
$('#formMensagem').on('submit', function(e) {
  e.preventDefault();
  const amigo_id = $('input[name="amigo_id"]').val();
  const mensagem = $('input[name="mensagem"]').val().trim();

  if (mensagem === '') return;

  $.post('enviar_mensagem.php', { amigo_id, mensagem }, function() {
...
// Atualizar mensagens automaticamente a cada 3s
