...
// Busca fóruns da comunidade
$stmt = $conn->prepare("SELECT * FROM foruns WHERE comunidade_id = ? ORDER BY id DESC");
$stmt->bind_param("i", $comunidade_id);
$stmt->execute();
$foruns = $stmt->get_result();
$stmt->close();
...
<head>
    <meta charset="UTF-8">
    <title>Fóruns - <?= htmlspecialchars($comunidade['nome']) ?></title>
...
<div class="max-w-5xl mx-auto p-6">
    ...
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold">Fóruns de <?= htmlspecialchars($comunidade['nome']) ?></h1>
        <a href="ver_comunidade.php?id=<?= $comunidade_id ?>" 
           class="bg-gray-700 hover:bg-gray-600 px-4 py-2 rounded">Voltar</a>
    </div>

    <!-- Lista de fóruns -->
    <div class="space-y-4">
        <?php if ($foruns->num_rows > 0): ?>
            <?php while ($forum = $foruns->fetch_assoc()): ?>
                <div class="bg-gray-800 p-4 rounded-lg shadow">
                    <h2 class="text-lg font-bold text-indigo-400">
                        <?= htmlspecialchars($forum['titulo']) ?>
                    </h2>
                    <p class="text-gray-300"><?= nl2br(htmlspecialchars($forum['descricao'])) ?></p>
                    <div class="mt-2 text-sm text-gray-400">
                        Criado em <?= date("d/m/Y H:i", strtotime($forum['criado_em'] ?? 'now')) ?>
                    </div>
                    <a href="forum_ver.php?id=<?= $forum['id'] ?>" 
                       class="inline-block mt-3 bg-indigo-600 hover:bg-indigo-700 px-3 py-1 rounded">
                        Ver Discussão
                    </a>
                </div>
            <?php endwhile; ?>
        <?php else: ?>
            <p class="text-gray-400">Nenhum fórum criado ainda nesta comunidade.</p>
        <?php endif; ?>
    </div>
</div>
<!-- FOOTER -->
