...
// (aqui estamos considerando todos como "online" — você pode filtrar por last_activity se tiver)
$stmt = $conn->prepare("
...
$stmt->execute();
$amigosOnline = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
...
<nav class="fixed top-0 left-0 right-0 z-30 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 h-16 flex items-center justify-between px-6 shadow-sm">
  ...
  <div class="flex h-screen">
    ...
    <main class="flex-1 p-6 overflow-y-auto">
      <h1 class="text-2xl font-semibold mb-4">Amigos Online</h1>
      <?php if (empty($amigosOnline)): ?>
        <p class="text-gray-400">Nenhum amigo online no momento.</p>
      <?php else: ?>
        <ul class="space-y-4">
          <?php foreach ($amigosOnline as $amigo): ?>
            <li class="bg-gray-800 p-4 rounded-lg flex items-center justify-between">
              ...
              <a href="conversas.php?id=<?= $amigo['id'] ?>" class="bg-indigo-600 hover:bg-indigo-700 px-3 py-1 rounded">Conversar</a>
            </li>
          <?php endforeach; ?>
        </ul>
      <?php endif; ?>
